    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const current = allInsights.find(i => i.id === id)
                if (current) {
                    setFormData(current)
                } else {
                    alert("인사이트를 찾을 수 없습니다.")
    ...
    return (
        <div className="max-w-4xl space-y-8">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-slate-900">인사이트 수정</h1>
                    <p className="text-slate-500 mt-1">인사이트 정보를 수정합니다.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="flex justify-end pt-4">
                    <Button disabled={saving} className="h-14 px-10 rounded-2xl bg-primary hover:bg-primary/90 text-white font-bold text-lg shadow-xl shadow-primary/20">
                        {saving ? <Loader2 className="h-6 w-6 animate-spin" /> : <><Save className="h-6 w-6 mr-2" /> 수정사항 저장</>}
                    </Button>
                </div>
            </form>
        </div>
